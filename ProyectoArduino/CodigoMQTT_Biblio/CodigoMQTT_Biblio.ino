...
//MQTT -- Topicos.
const char* lum1_topic = "smarlib/sillas/ilum1";
const char* lum2_topic = "smarlib/sillas/ilum2";
const char* lum3_topic = "smarlib/sillas/ilum3";
const char* lum4_topic = "smarlib/sillas/ilum4";

const char* temperature_topic = "smarlib/temperatura";
...
void connect_MQTT(){
  ...
  if (client.connect(clientID, mqtt_username, mqtt_password)) {
    ...
    client.subscribe("smarlib/sillas/ilum1");
    client.subscribe("smarlib/sillas/ilum2");
    client.subscribe("smarlib/sillas/ilum3");
    client.subscribe("smarlib/sillas/ilum4");
  }
  else {
...
void loop() {
  ...
  temperatura = 28.3;
  ultrasonidos = funcUltrasonidos();
  espalda = funcEspalda();

  //Cambio de Estado LED según Ultrasonido --------------------------------------------------------------
  if (ultrasonidos == 3){
    digitalWrite(ledPinR1, LOW)
  }else if(ultrasonids == 1 || ultrasonidos == 2){
  ...
  String us="Temp: "+String((int)ultrasonidos)+" Eu ";
  ...
  if (client.publish(ultrasound_topic, String(ultrasonidos).c_str())) {
    ...
    client.connect(clientID, mqtt_username, mqtt_password);
    delay(10); // This delay ensures that client.publish doesn't clash with the client.connect call
    client.publish(ultrasound_topic, String(ultrasonidos).c_str());
  }
